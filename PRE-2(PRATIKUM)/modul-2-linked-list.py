...
def tambah_node (head, data):
      ...
      if head is None:
            return new_node # menjadikan new_node sebagai head
      # jika list tidk kosong
      current = head 
...
# function menampilokan linked list
...
# cara menerapkan function
head = None # supaya list kosong

# tambah node
head = tambah_node(head, 10)
head = tambah_node(head, 90)
head = tambah_node(head, 1000)
...
# praktek 28
# Menghapus node di posisi manapun (tengah)
# membuat node baru
def sisip_depan(head, data):
    new_node = {'data': data, 'next': head}
    return new_node
